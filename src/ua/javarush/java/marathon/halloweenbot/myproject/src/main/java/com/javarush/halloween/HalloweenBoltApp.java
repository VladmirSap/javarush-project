...
public class HalloweenBoltApp extends com.javarush.halloween.SimpleTelegramBot {

    private AIService aiService = new AIService();
    private AppMode mode;
    private String imageType = "create_anime";
    ArrayList<Path> imageList = new ArrayList<>();
    ...
    public void editMessage() {
        ...
        if(!Files.exists(root.resolve(photoPath))) {
            sendTextMessage("Спочатку створить або завантажте зображення");
            return;
        }
    ...
    public void mergeButtonCallback() {
        if (imageList.size() < 2) {
            sendTextMessage("Спочатку завантажте ваше фото");
            return;
        }
